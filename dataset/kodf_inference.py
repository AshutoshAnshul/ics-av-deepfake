...
class KoDFInference(Dataset):
    ...
    def __getitem__(self, index):
        ...
        filename = '_'.join(str(meta).split('/')[-2:])

        feature_file_path = os.path.join(self.root, 'decoder_lrs2_feat', filename)
        
        filePath = str(meta).split('.')[0] + '_merged.mp4'
...
class KoDFInferenceDataModule(LightningDataModule):
    # train_dataset = FakeAVCelebPretrain
    val_dataset = KoDFInference
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        self.val_dataset = KoDFInference(subset=self.val_subset, root=self.root, frame_padding=self.frame_padding, fps=self.fps, img_size=self.img_size, video_transform=self.video_transform, audio_transform=self.audio_transform, sample=self.sample, pad_to_max_len=self.pad_to_max_len, take_factor=self.val_take_factor, num_dists=self.num_dists)
