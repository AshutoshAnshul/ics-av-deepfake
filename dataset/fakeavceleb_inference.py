...
from utils import read_video, padding_video, padding_audio, resize_video, read_json
...
class FakeAVCelebPretrain(Dataset):
    def __init__(self, subset: str, root: str = "data", frame_padding: int = 512, fps: int = 25, img_size = 96, take_factor = 4, num_dists = 32,
        ...
        metadata_file = os.path.join(src_dir, "FakeAVCeleb/meta_data.json")

        assert os.path.isfile(metadata_file), f"No file name {metadata_file} found" 

        self.metadata : List[Metadata] = read_json(metadata_file, lambda x: Metadata(**x))          
    ...
    def __getitem__(self, index):
        ...
        feature_file_path = os.path.join(self.root, 'FakeAVCeleb/decoder_voxceleb_feat', str(meta.feature_file))
        
        filePath = os.path.join(self.root, str(meta.path), str(meta.filename))
...
class FakeAVCelebPretrainDataModule(LightningDataModule):
    # train_dataset = FakeAVCelebPretrain
    val_dataset = FakeAVCelebPretrain
