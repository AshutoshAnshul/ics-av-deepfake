...
class DFDCClassification(Dataset):
    def __init__(self, subset: str, root: str = "data", frame_padding: int = 512, take_factor = 4, num_dists = 32, pad_to_max_len: bool = True, leave_section='None') :
        ...
        with open(metadata_file, 'r') as meta_file:
            self.metadata : List[str] = json.load(meta_file)
    ...
    def __getitem__(self, index):
        ...
        filename = str(meta['file']).replace('.mp4', '.pkl')
...
class DFDCClassificationDataModule(LightningDataModule):
    train_dataset = DFDCClassification
    val_dataset = DFDCClassification
    test_dataset = DFDCClassification
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        self.test_dataset = DFDCClassification(subset=self.test_subset, root=self.root, frame_padding=self.frame_padding, pad_to_max_len=self.pad_to_max_len, take_factor=self.val_take_factor, num_dists=self.num_dists, leave_section=self.leave_section)
