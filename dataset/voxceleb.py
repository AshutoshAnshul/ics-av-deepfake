...
class Voxceleb(Dataset):
    def __init__(self, subset: str, root: str = "data", frame_padding: int = 512, fps: int = 25, img_size = 96, take_factor = 4, num_dists = 32,
        ...
        metadata_file = os.path.join(src_dir, subset, "filtered_file_ffmpeg_next.pkl")

        if os.path.isfile(metadata_file):
            with open(metadata_file, 'rb') as meta_file:
                self.metadata = pickle.load(meta_file)
        else:
            if ('dev' in subset) and sample:
                metadata = list(Path(src_dir / subset).rglob("*.mp4"))

                # Dictionary to store sampled videos
                sampled_videos = {}

                for path in metadata:
                    # Split the path to extract person_id and video_id
                    parts = str(path).strip().split(os.sep)
                    person_id = parts[-3]
                    video_id = parts[-2]
                    
                    # Create a unique key for each person_id and video_id combination
                    key = (person_id, video_id)
                    
                    # Sample only one video for each video_id
                    if key not in sampled_videos:
                        sampled_videos[key] = path

                self.metadata = list(sampled_videos.values())
                del metadata
                del sampled_videos
            else:
                self.metadata = list(Path(src_dir / subset).rglob("*.mp4"))
            
            with open(metadata_file, 'wb') as meta_file:
                pickle.dump(self.metadata, meta_file)
        
        sample_size = math.ceil(len(self.metadata)/take_factor)
    ...
    def __getitem__(self, idx):
        ...
        filePath = os.path.join(self.root, '/'.join(filePath.split('/')[-5:]))
