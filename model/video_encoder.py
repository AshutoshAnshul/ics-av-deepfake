#modified from https://github.com/ControlNet/LAV-DF/blob/master/model/video_encoder.py
...
from model.resnet_avhubert import ResEncoder
...
class VitFrameLevel(Module):
    def __init__(self, v_cla_feature_in: int = 256, vit_type = "vit_t"):
        super().__init__()
        if vit_type == "vit_t":
            self.hidden_dim = 192
        ...
        self.block.heads = None

        self.output_proj = Linear(self.hidden_dim, v_cla_feature_in)

    def forward(self, x: Tensor):
        _, _, t, _, _ = x.shape
        x = rearrange(x, 'b c t h w -> (b t) c h w')
        # Reshape and permute the input tensor
        x = self.block._process_input(x)
        n = x.shape[0]

        # Expand the class token to the full batch
        batch_class_token = self.block.class_token.expand(n, -1, -1)
        x = cat([batch_class_token, x], dim=1)

        x = self.block.encoder(x)

        x = x[:, 0]
        
        x = rearrange(x, '(b t) f -> b t f', t=t)
...
def get_video_encoder(v_cla_feature_in, temporal_size, v_encoder, segment_size=5):
    if v_encoder == "mvit_t":
    ...
    elif v_encoder == "frame_vit_t":
        video_encoder = VitFrameLevel(v_cla_feature_in=v_cla_feature_in, vit_type="vit_t")
    elif v_encoder == "frame_vit_s":
        video_encoder = VitFrameLevel(v_cla_feature_in=v_cla_feature_in, vit_type="vit_s")
    elif v_encoder == "resnet":
        video_encoder = ResEncoder(frontend_temporal_kernel_size=segment_size, out_features=v_cla_feature_in)
    else:
