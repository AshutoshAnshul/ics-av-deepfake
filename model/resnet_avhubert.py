...
class ResEncoder(nn.Module):
    def __init__(self, frontend_temporal_kernel_size=5, out_features=256, relu_type='relu'):
        super(ResEncoder, self).__init__()
        self.frontend_nout = 64
        frontend_relu = nn.PReLU(num_parameters=self.frontend_nout) if relu_type == 'prelu' else nn.ReLU()
        self.frontend3D = nn.Sequential(
            nn.Conv3d(3, self.frontend_nout, kernel_size=(frontend_temporal_kernel_size, 7, 7), stride=(1, 2, 2), padding=(frontend_temporal_kernel_size//2, 3, 3), bias=False),
            nn.BatchNorm3d(self.frontend_nout),
            frontend_relu,
            nn.MaxPool3d( kernel_size=(1, 3, 3), stride=(1, 2, 2), padding=(0, 1, 1)))
        self.trunk = ResNet(BasicBlock, [2, 2, 2, 2], relu_type=relu_type)
        self.out_proj = nn.Linear(512, out_features)
